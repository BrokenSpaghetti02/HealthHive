/**
 * API Client for HealthHive Mobile App
 */
...
class ApiClient {
  ...
  // Patient endpoints
  async getPatients(params?: { barangay?: string; risk_level?: string; skip?: number; limit?: number }) {
  ...
  async bulkSyncVisits(visits: any[]) {
    const response = await this.request('/api/visits/bulk-sync', {
      method: 'POST',
